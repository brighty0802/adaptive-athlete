...
export function AdherencePreview({ today, sessions, workouts }: { today: string; sessions: readonly CompletedWorkout[]; workouts: readonly WorkoutSummary[] }) {
  ...
  return (
    <section className="adherence panel" aria-labelledby="adherence-heading">
      ...
      <p className="calendar-month">{formatCalendarDate(today, { month: "long", year: "numeric" })} <span>· Sample activity</span></p>
      <div className="calendar" role="list" aria-label="Monthly training activity">
        ...
        {Array.from({ length: dayCount }, (_, index) => {
          ...
          const session = monthSessions.find((item) => item.date === date);
          const workout = workouts.find((item) => item.id === session?.workoutId);
          const label = `${formatCalendarDate(date, { day: "numeric", month: "long" })}: ${workout ? `${workout.name} completed` : date > today ? "Upcoming" : "No workout"}${date === today ? ", today" : ""}`;
